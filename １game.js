...
const ENEMY_TYPES = [
  ...
  { time: 42000, name: 'BulletQueen', hp: 25000, speed: 50, size: 40, color: '#ff69b4', exp: 1200, type: 'BulletQueenBoss' },
  { time: 54000, name: 'TeleportHunter', hp: 28000, speed: 250, size: 35, color: '#ffff00', exp: 1400, type: 'TeleportHunterBoss' },
  { time: 66000, name: 'ArcMage', hp: 40000, speed: 300, size: 45, color: '#00ccff', exp: 1600, type: 'ArcMageBoss' },
  { time: 0, name: 'god', hp: 50000, speed: 300, size: 45, color: '#f0f8ff', exp: 1200, type: 'godBoss' }
];
const bossTypes = ['BulletQueenBoss', 'ArcMageBoss', 'TeleportHunterBoss','godBoss'
];
// 3. 特別ドロップ
const DROPS = [
...
const GAME_EVENTS = [
    {id : 1 ,time: 10, name: '', effect: () => { enemies = []; spawnBoss('god') }, color: 'rgba(0, 0, 0, 0)' },
...
case 0:
    ...
    if (e.fireTimer <= 0) {
        ...
        const dirIndex = Math.floor(e.subState / 3) % 4;
        const a = baseAngles[dirIndex];

        const radius = 300;

        const spawnX = player.x + Math.cos(a) * radius;
        const spawnY = player.y + Math.sin(a) * radius;
    ...
    case 1:
        ...
        if (e.fireTimer <= 0) {
            ...
            // 上からランダム落下
            let rx = player.x + (Math.random() - 0.5) * 10;
            fireEnemyBullet({x:rx, y:e.y-10}, true, Math.PI/2, 'big', 50);

            // プレイヤー狙い
            fireEnemyBullet(e, true, angleToPlayer, 'normal', 70);
    ...
    case 2:
        ...
        if (e.fireTimer <= 0) {
            ...
            // 周回移動
            e.tempAngle += (Math.random() < 0.5 ? 1 : -1) * 0.02;
            e.x = player.x + Math.cos(e.tempAngle) * 5;
            e.y = player.y + Math.sin(e.tempAngle) * 5;
    ...
    case 3:
        ...
        if (e.subTimer >= 0.5) {
            ...
            e.x = player.x + Math.cos(ang)*r;
            e.y = player.y + Math.sin(ang)*r;

            fireEnemyBullet(e, true, angleToPlayer, 'bigSplit', 40);
    ...
    case 4:
        ...
        if (e.subState === 2) {
            ...
            // 円形弾
            for (let i = 0; i < 4; i++) {
                fireEnemyBullet(e, true, i*Math.PI/2, 'normal', 50);
            }
